...
vai_filter = filters.create(lambda _, __, update: set(update.text.lower().split()[1::2]) == {'vai'})
...
def vai_handler(client: Client, message: Message):
    ...
    if rigged.randint(0, 49) == 0:
        ...
        client.send_message(chat_id=message.chat.id, text=rigged.choice(answers))
    else:
        client.send_message(chat_id=message.chat.id, text=rigged.choice(alternatives))
...
def onko_pakko_handler(client: Client, message: Message):
    ...
    groups = re.match(r"^onko pakko ([^?]+)(\??)$", message.text.lower(), re.IGNORECASE)
    data = [
        message.from_user.id,+
        now.day,
        now.month,
    ...
    if rigged.randint(0, 1) == 0:
        client.send_message(chat_id=message.chat.id, text='ei ole pakko {}'.format(groups.group(1)))
    else:
        client.send_message(chat_id=message.chat.id, text='on pakko {}'.format(groups.group(1)))
